...
The committed sample is available at `assets/sample-recording.json`. In the app, **Load sample** switches from demo/live mode to replay and stops the previous source before playback begins. The committed sample expands captured fixture waveforms into a 30-second, demo-like preview window so grid and 3D review stays visibly active. Replay uses the same raster, heatmap, waveform, 3D, reset, pause, and channel-selection paths as live data.

Create a snapshot from an existing fixture capture:

```bash
python3 tools/export_recording.py --fixtures test/fixtures --out assets/sample-recording.json --preview-duration-ms 30000 --preview-cycle-ms 100
```
